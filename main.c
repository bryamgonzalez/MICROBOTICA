...
static const char *TAG = "MODULO2_ESP1";

static const gpio_num_t led_1_pin = GPIO_NUM_15;
static const gpio_num_t led_2_pin = GPIO_NUM_4;
static const gpio_num_t button_1_pin = GPIO_NUM_22;
static const gpio_num_t button_2_pin = GPIO_NUM_23;
...
static void print_pin_summary(void) {
    ESP_LOGI(TAG, "Modulo 2 - ESP1");
    ESP_LOGI(TAG, "LED 1 -> GPIO 15");
    ESP_LOGI(TAG, "LED 2 -> GPIO 4");
    ESP_LOGI(TAG, "Boton 1 -> GPIO 22 (envia 'A')");
    ESP_LOGI(TAG, "Boton 2 -> GPIO 23 (envia 'B')");
...
static void handle_received_char(char received) {
    if (received == 'A') {
        set_leds(true, false);
        ESP_LOGI(TAG, "Recibido 'A' -> LED1 encendido");
    } else if (received == 'B') {
        set_leds(false, true);
        ESP_LOGI(TAG, "Recibido 'B' -> LED2 encendido");
    } else {
